const fs = require('fs');
const lines = fs.readFileSync('src/modules/auth/pages/Login.jsx', 'utf8').split('\n');
            ...
            {step === 1 ? (
              <>
                <div className="flex items-center justify-center gap-3">
                  <div className="w-[110px] h-[110px] bg-[#FF5A1F] rounded-[30px] flex flex-col items-center justify-center font-black text-3xl leading-[1] tracking-tighter shadow-lg shrink-0">
                    <span className="text-black">Door</span>
                    <span className="text-white">Dish</span>
                  </div>
                  <div className="w-[120px] h-[120px] shrink-0">
                    <img src="/assets/images/MapRider.png" alt="Rider" className="w-full h-full object-contain drop-shadow-lg" />
                  </div>
                </div>
                <div className="mt-5">
              ...
              <div className="mt-8 flex flex-col items-center">
                <div className="w-[80px] h-[80px] bg-[#FF5A1F] rounded-[24px] flex flex-col items-center justify-center font-black text-2xl leading-[1] tracking-tighter shadow-lg mb-6">
                  <span className="text-black">Door</span>
                  <span className="text-white">Dish</span>
                </div>
            ...
            <AnimatePresence mode="wait">
              {step === 1 ? (
                <motion.form key="step-1" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} onSubmit={handleSendOTP} className="space-y-5">
                  <div className={\`flex items-center h-[56px] rounded-full border-2 \${error || blockTimer > 0 ? 'border-red-500 bg-red-50' : 'border-[#FF5A1F] bg-white'} px-2 transition-colors\`}>
...
lines.splice(506, 436, newRender);
fs.writeFileSync('src/modules/auth/pages/Login.jsx', lines.join('\n'), 'utf8');
