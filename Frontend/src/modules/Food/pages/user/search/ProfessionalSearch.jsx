      ...
      <div className="sticky top-0 z-50 bg-white/80 dark:bg-zinc-900/80 backdrop-blur-xl border-b border-gray-100 dark:border-zinc-800 px-3 py-2 sm:px-4 sm:py-3">
        <div className="max-w-4xl mx-auto flex items-center gap-2 sm:gap-3">
          ...
          <div className="flex-1 relative group">
            <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-[#FF5A1F] transition-transform group-focus-within:scale-110" strokeWidth={2.5} />
            <Input 
              autoFocus
              placeholder={isTakeawaySearch ? "Search takeaway restaurants..." : "Search dishes or restaurants"} 
              value={query}
              onChange={(e) => setQuery(e.target.value)}
              className="pl-10 pr-12 h-10 sm:h-12 bg-gray-50 dark:bg-zinc-800/50 border-gray-100 dark:border-zinc-700 focus:border-[#FF5A1F] dark:focus:border-[#FF5A1F] focus:ring-4 focus:ring-[#FF5A1F]/5 rounded-2xl text-sm sm:text-base transition-all"
            />
            ...
            <div className="absolute right-1.5 top-1/2 -translate-y-1/2 flex items-center gap-1">
              ...
              <button 
                ...
                className={`p-1.5 rounded-xl transition-all active:scale-95 ${isListening ? 'bg-red-50 text-red-500 animate-pulse' : 'text-[#FF5A1F]'}`}
              >
          ...
          <div className="mb-8">
            <div className="flex items-center justify-between mb-5 px-1">
              ...
              {visibleCategories.length > 8 && (
                <span className="text-[10px] font-bold text-[#FF5A1F] uppercase tracking-tighter">Swipe for more</span>
              )}
            </div>
            {categories.length === 0 ? (
              ...
              <div className="grid grid-cols-4 sm:grid-cols-5 md:grid-cols-6 lg:grid-cols-8 gap-x-3 gap-y-6">
                {visibleCategories.map((cat) => (
                  ...
                  >
                    <div className={`relative w-15 h-15 sm:w-16 sm:h-16 rounded-[22px] mb-2 shadow-sm border-2 transition-colors duration-150 ${selectedCategoryId === cat._id ? 'border-[#FF5A1F] shadow-md shadow-[#FF5A1F]/5' : 'border-gray-50 dark:border-zinc-800 bg-white dark:bg-zinc-900 group-hover:border-gray-200'}`}>
                    ...
                    <span className={`text-[10px] sm:text-[11px] font-bold text-center line-clamp-1 transition-colors duration-150 ${selectedCategoryId === cat._id ? 'text-[#FF5A1F]' : 'text-gray-500 dark:text-zinc-400 group-hover:text-gray-800'}`}>
                      {cat.name}
                    </span>
                  </button>
                ))}
                  ...
                  {results.dishes.map((r) => (
                    <Link to={`/food/user/restaurants/${r.slug || r._id}${r.matchedDishId ? `?dish=${r.matchedDishId}` : ''}`} key={r._id} className="flex gap-4 p-3 bg-white dark:bg-zinc-900 rounded-[24px] shadow-sm border border-gray-100 dark:border-zinc-800 hover:shadow-xl hover:shadow-gray-200/50 dark:hover:shadow-none transition-all group overflow-hidden active:scale-[0.98]">
                       ...
                       <div className="flex-1 min-w-0 py-1">
                          <div className="text-[#F87171] text-[9px] font-black uppercase tracking-wider mb-1 px-2 py-0.5 bg-[#FF5A1F]/5 rounded-full w-fit">
                             {r.matchedDish || query}
                          </div>
                          <h3 className="text-base font-black text-gray-900 dark:text-white line-clamp-1 group-hover:text-[#FF5A1F] transition-colors">{r.restaurantName}</h3>
                          <div className="flex items-center gap-3 text-[11px] text-gray-500 dark:text-zinc-400 mt-2 font-medium">
                             <div className="flex items-center gap-1">
                                <Star className="w-3 h-3 text-[#FF5A1F] fill-[#FF5A1F]" />
                  ...
                  {results.restaurants.map((r) => (
                    <Link to={`/food/user/restaurants/${r.slug || r._id}`} key={r._id} className="block group active:scale-[0.98] transition-all">
                      <div className="relative rounded-[32px] overflow-hidden aspect-[16/10] sm:aspect-[16/9] mb-4 bg-gray-100 dark:bg-zinc-800 shadow-xl shadow-gray-200/20">
                        ...
                        {r.offer && (
                           <div className="absolute top-5 left-0 bg-[#FF5A1F] text-white text-[10px] font-black px-4 py-2 rounded-r-2xl shadow-xl flex items-center gap-1.5 tracking-tighter uppercase whitespace-nowrap">
                      ...
                      <div className="flex items-center justify-between px-2">
                         ...
                         <div className="text-[10px] font-black text-white bg-gradient-to-r from-[#FF5A1F] to-[#F87171] px-3 py-1.5 rounded-full uppercase tracking-widest shadow-lg shadow-[#FF5A1F]/20">
                            View Menu
                         </div>
                      </div>
                    </Link>
