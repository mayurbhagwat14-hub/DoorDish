          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <ScrollReveal delay={0.1}>
              <Card>
                ...
                <CardContent className="space-y-4">
                  {orderType === "takeaway" ? (
                    <div className="border-2 border-[#FF5A1F] bg-orange-50 rounded-lg p-4">
                      <div className="flex items-start justify-between">
                        <div className="flex items-center gap-2 mb-2">
                          <ShoppingBag className="h-4 w-4 text-[#FF5A1F]" />
                          ...
                          >
                            <div className="flex items-start justify-between">
                              <div className="flex-1">
                                {address.isDefault && (
                                  <Badge className="mb-2 bg-[#FF5A1F] text-white">Default</Badge>
                                )}
                                <p className="text-sm font-medium">{addressString}</p>
                              </div>
                              {isSelected && (
                                <CheckCircle className="h-5 w-5 text-[#FF5A1F]" />
                              )}
                            </div>
                          </div>
                        )
            ...
            <ScrollReveal delay={0.2}>
              <Card>
                <CardHeader>
                  <CardTitle className="flex items-center gap-2">
                    <CreditCard className="h-5 w-5 text-[#FF5A1F]" />
                    Payment Method
                  </CardTitle>
                </CardHeader>
                <CardContent className="space-y-4">
                      ...
                      <div
                        className={`border-2 rounded-lg p-4 transition-colors ${
                          (userProfile?.isCodBlocked && isCodBlockingFeatureEnabled)
                            ...
                            : `cursor-pointer ${selectedPayment === "cod" ? "border-[#FF5A1F] bg-orange-50" : "border-gray-200 hover:border-orange-300"}`
                        }`}
                        onClick={() => {
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              {userProfile?.isCodBlocked && isCodBlockingFeatureEnabled ? (
                                <p className="text-xs text-[#FF5A1F] mt-1 font-medium">Not available due to multiple cancellations</p>
                              ) : (
                        ...
                        >
                          <div className="flex items-start justify-between">
                            <div className="flex-1">
                              <div className="flex items-center gap-2 mb-2">
                                {payment.isDefault && (
                                  <Badge className="bg-[#FF5A1F] text-white">Default</Badge>
                                )}
            ...
            <ScrollReveal delay={0.25}>
              <Card>
                <CardHeader>
                  <CardTitle className="flex items-center gap-2">
                    <MessageSquare className="h-5 w-5 text-[#FF5A1F]" />
                ...
                <CardContent className="space-y-4 md:space-y-6">
                  ...
                  <div className="space-y-2 md:space-y-3 pt-4 md:pt-6 border-t dark:border-gray-700">
                    ...
                    <div className="flex justify-between font-bold text-lg md:text-xl lg:text-2xl pt-2 md:pt-3 border-t dark:border-gray-700">
                      ...
                      <span className="text-[#FF5A1F] dark:text-orange-400">₹{total.toFixed(0)}</span>
                    </div>
                  </div>

                  <Button
                    className="w-full bg-[#FF5A1F] hover:bg-[#E64A0F] text-white mt-4 md:mt-6 h-11 md:h-12 text-sm md:text-base border-none"
