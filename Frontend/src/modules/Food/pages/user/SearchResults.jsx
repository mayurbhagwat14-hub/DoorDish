          ...
          >
            {categories.map((cat) => {
              ...
              return (
                ...
                >
                  {isAllCategory ? (
                    <div className={`w-16 h-16 rounded-full border-2 transition-all flex items-center justify-center ${isSelected ? 'border-[#FF5A1F] dark:border-[#FF5A1F] shadow-lg bg-[#F9F9FB] dark:bg-[#FF5A1F]/20' : 'border-gray-200 dark:border-gray-700 bg-white dark:bg-[#222222]'}`}>
                      <Grid2x2 className={`h-6 w-6 ${isSelected ? 'text-[#FF5A1F]' : 'text-gray-500 dark:text-gray-400'}`} />
                    </div>
                  ) : cat.image ? (
                    <div className={`w-16 h-16 rounded-full overflow-hidden border-2 transition-all ${isSelected ? 'border-[#FF5A1F] dark:border-[#FF5A1F] shadow-lg' : 'border-transparent'
                      }`}>
                      <img
                        src={cat.image}
                        alt={cat.name}
                    ...
                    <div className={`w-16 h-16 rounded-full bg-gray-100 dark:bg-gray-800 flex items-center justify-center border-2 transition-all ${isSelected ? 'border-[#FF5A1F] dark:border-[#FF5A1F] shadow-lg bg-[#F9F9FB] dark:bg-[#FF5A1F]/20' : 'border-transparent'
                      }`}>
                      <span className="text-xl">???</span>
                    </div>
                  )}
                  <span className={`text-xs font-medium whitespace-nowrap ${isSelected ? 'text-[#FF5A1F] dark:text-[#FF5A1F]' : 'text-gray-600 dark:text-gray-400'
                    }`}>
                    {cat.name}
                  </span>
                </button>
              ...
              return (
                <Button
                  ...
                  className={`h-9 px-3 rounded-lg flex items-center gap-1.5 whitespace-nowrap flex-shrink-0 transition-all font-medium ${isActive
                    ? 'bg-[#FF5A1F] text-white border-[#FF5A1F] hover:bg-[#E64A0F] dark:bg-[#FF5A1F] dark:hover:bg-[#E64A0F]'
                    : 'bg-white dark:bg-[#1a1a1a] border border-gray-200 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800 text-gray-600 dark:text-gray-300'
                    }`}
                >
                  {filter.hasIcon && filter.id === 'price-match' && (
                    <span className={`text-xs ${isActive ? 'text-white' : 'text-[#FF5A1F] dark:text-[#FF5A1F]'}`}>?</span>
                  )}
                  ...
                  >
                    <div className={`group ${shouldShowGrayscale ? 'grayscale opacity-75' : ''}`}>
                      ...
                      <div className="relative aspect-square rounded-xl overflow-hidden mb-2 bg-gray-200 dark:bg-gray-800">
                        ...
                        {restaurant.offer && (
                          <div className="absolute top-1.5 left-1.5 bg-gradient-to-r from-[#FF5A1F] to-[#E64A0F] text-white text-[10px] font-semibold px-1.5 py-0.5 rounded">
                            {restaurant.offer}
                          </div>
                        )}
                      </div>
                    ...
                    <CardContent className="p-3 sm:p-4 lg:p-5 flex flex-col flex-grow">
                      ...
                      {restaurant.offer && (
                        <div className="flex items-center gap-2 text-sm lg:text-base mt-auto">
                          <BadgePercent className="h-4 w-4 lg:h-5 lg:w-5 text-[#FF5A1F] dark:text-[#FF5A1F]" strokeWidth={2} />
