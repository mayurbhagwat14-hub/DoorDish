      ...
      <AnimatePresence mode="popLayout">
        <div className="flex gap-2 sm:gap-4 md:gap-5 px-4 md:px-0 py-2">
          {categories.map((category, index) => (
            ...
            >
              <div className="relative w-16 h-16 sm:w-20 sm:h-20 md:w-24 md:h-24 lg:w-28 lg:h-28 rounded-full bg-white dark:bg-[#1a1a1a] shadow-md group-hover:shadow-xl transition-all duration-300 p-1 ring-2 ring-transparent group-hover:ring-[#FF5A1F]/20 overflow-hidden">
                <OptimizedImage
                  src={category.image}
                  alt={category.name}
              ...
              <span className="text-[10px] sm:text-xs md:text-sm font-semibold text-gray-700 dark:text-gray-300 group-hover:text-[#FF5A1F] text-center whitespace-normal max-w-[80px] sm:max-w-[100px] leading-tight line-clamp-2 transition-colors duration-300">
                {category.name}
              </span>
            </motion.div>
          ))}
