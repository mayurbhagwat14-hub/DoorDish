...
const restaurantRegisterSchema = z.object({
    ...
    panNumber: z
        .preprocess((val) => (typeof val === 'string' ? val.trim().toUpperCase() : val),
        z.string().regex(panRegex, 'Invalid PAN format').optional().or(z.literal(''))),
...
export const validateRestaurantRegisterDto = (body) => {
    ...
    if (openingMinutes !== null && closingMinutes !== null && data.openingTime && data.closingTime) {
